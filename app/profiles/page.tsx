...
export default async function ProfilesPage({ searchParams }: PageProps) {
  ...
  if (districtFilter) {
    filtered = filtered.filter((p: any) =>
      ...
      p.city === districtFilter ||
      p.location === districtFilter
    )
  }
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 16px' }}>
        ...
        <form method="GET" action="/profiles" style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            ...
            <select name="district" defaultValue={districtFilter} style={{
              padding: '8px 12px', borderRadius: '8px', border: '1.5px solid #e5e7eb',
              fontSize: '13px', color: '#1f2937', background: 'white', minWidth: '140px'
            }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            ...
            <select name="minAge" defaultValue={String(minAge)} style={{
              padding: '8px 12px', borderRadius: '8px', border: '1.5px solid #e5e7eb',
              fontSize: '13px', color: '#1f2937', background: 'white'
            }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            ...
            <select name="maxAge" defaultValue={String(maxAge)} style={{
              padding: '8px 12px', borderRadius: '8px', border: '1.5px solid #e5e7eb',
              fontSize: '13px', color: '#1f2937', background: 'white'
            }}>
              {Array.from({ length: 53 }, (_, i) => i + 18).map(a => (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            ...
            <select name="marital" defaultValue={maritalFilter} style={{
              padding: '8px 12px', borderRadius: '8px', border: '1.5px solid #e5e7eb',
              fontSize: '13px', color: '#1f2937', background: 'white'
            }}>
          ...
          <button type="submit" style={{
            padding: '8px 20px', background: 'linear-gradient(135deg,#e11d48,#db2777)',
        ...
        {paginatedProfiles.length > 0 ? (
              ...
              <div style={{
                ...
                {Array.from({ length: Math.min(totalPages, 10) }, (_, i) => i + 1).map(pageNum =>
                  pageNum > FREE_MAX_PAGES ? (
                    <Link key={pageNum} href="/pricing" style={{
                      padding: '8px 14px', background: '#f9fafb', border: '2px solid #e5e7eb',
                      borderRadius: '8px', fontWeight: 700, color: '#d1d5db',
                      textDecoration: 'none', fontSize: '13px'
                    }} title="Upgrade to access">
                      {pageNum}
                    </Link>
                  ) : (
                    <Link key={pageNum} href={buildUrl(pageNum)} style={{
                      padding: '8px 14px',
                      background: currentPage === pageNum
                        ? 'linear-gradient(135deg,#e11d48,#db2777)'
                        : 'white',
                      border: currentPage === pageNum ? '2px solid #e11d48' : '2px solid #e5e7eb',
                ...
                {currentPage >= FREE_MAX_PAGES && (
                  <Link href="/pricing" style={{
                    ...
                    borderRadius: '8px', fontWeight: 700, color: 'white',
                    textDecoration: 'none', fontSize: '13px'
                  }}>
                    Upgrade for More
                  </Link>
          ...
          <div style={{
            background: 'white', borderRadius: '20px', padding: '64px 24px',
            textAlign: 'center', boxShadow: '0 2px 12px rgba(0,0,0,0.06)'
          }}>
            <p style={{ fontSize: '20px', fontWeight: 700, color: '#9ca3af', margin: '0 0 8px' }}>
