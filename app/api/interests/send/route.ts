...
export async function POST(req) {
  try {
    ...
    const { data: sender } = await supabase.from("profiles").select("package").eq("id", senderId).single();
    if (sender?.package === "prottasha") {
      const monthStart = new Date(); monthStart.setDate(1); monthStart.setHours(0,0,0,0);
      const { count } = await supabase.from("interests").select("*", { count: "exact", head: true }).eq("sender_id", senderId).gte("sent_at", monthStart.toISOString());
    ...
    const { data, error } = await supabase.from("interests").insert([{ sender_id: senderId, receiver_id: receiverId, status: "pending", is_seed_receiver: isSeed }]).select("id").single();
