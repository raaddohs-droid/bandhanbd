...
export default function ProfileCard({ 
  ...
  // Can view contact if: user is verified OR user has premium/elite package
              ...
              {canViewContact ? (
                ...
                <>
                  <p className="text-sm text-gray-700 mb-1">
                    <strong>Phone:</strong> {maskPhone(profile.phone)}
                  </p>
                  {profile.email && (
                    <p className="text-sm text-gray-700 mb-3">
                      <strong>Email:</strong> {maskEmail(profile.email)}
                    </p>
                  )}
          ...
          {isUserFree && !currentUserVerified && (
            <div className="mt-4 p-4 bg-gradient-to-r from-yellow-50 to-orange-50 border-2 border-yellow-300 rounded-xl">
              ...
              <div className="grid grid-cols-2 gap-2">
                <Link
                  href="/pricing"
                  className="px-3 py-2 bg-gradient-to-r from-rose-500 to-pink-600 text-white rounded-lg text-xs font-bold hover:shadow-lg transition text-center"
                >
                  Upgrade ৳999/mo
                </Link>
                <Link
              ...
              <>
                ...
                {currentPhotoIndex > 0 && (
                  <button
                    ...
                    className="absolute left-4 top-1/2 -translate-y-1/2 bg-white/20 hover:bg-white/30 text-white rounded-full w-12 h-12 flex items-center justify-center text-2xl"
                  >
                    ←
                  </button>
                )}
                ...
                {currentPhotoIndex < allPhotos.length - 1 && (
                  <button
                    onClick={() => setCurrentPhotoIndex(currentPhotoIndex + 1)}
                    className="absolute right-4 top-1/2 -translate-y-1/2 bg-white/20 hover:bg-white/30 text-white rounded-full w-12 h-12 flex items-center justify-center text-2xl"
                  >
                    →
                  </button>
                )}
